...
namespace E_shopLib1
{
    public class PrintableInvoiceCreator
    {
            ...
            html.AppendLine($"            <div class=\"invoice-number\">№ {EscapeHtml(invoice.SerialNumber)}</div>");
            ...
            if (invoiceItems != null)
            {
                ...
                {
                    html.AppendLine("                <tr>");
                    html.AppendLine($"                    <td>{EscapeHtml(item.Article)}</td>");
                    html.AppendLine($"                    <td>{EscapeHtml(item.Name)}</td>");
                    html.AppendLine($"                    <td>{EscapeHtml(item.Category)}</td>");
                    html.AppendLine($"                    <td>{item.Quantity}</td>");
                    html.AppendLine($"                    <td>{item.Price:N2}</td>");
                    html.AppendLine($"                    <td>{EscapeHtml(item.Unit)}</td>");
